...
本実験は，**訓練・圧縮フェーズ**，**デプロイフェーズ**，**評価フェーズ**の3つのフェーズから構成されます。

#### フェーズ1: 訓練・圧縮フェーズ

PC上でPyTorchを用いてニューラルネットワークを訓練し，CSR形式への変換および量子化を適用してパラメータファイルを生成します。

**ステップ1: 環境セットアップとデータ準備**

1. **Jupyter Notebookの起動**
   - `train_L1_normalization.ipynb`を開く

2. **Cell 1-2: パッケージのインストール**
   - PyTorch, NumPy, Matplotlib等の必要なライブラリをインストール
   - または，`pip install -r requirements.txt`で一括インストール

3. **Cell 3: ライブラリのインポート**
   - `cv2`, `torch`, `torch.nn`, `torch.optim`, `numpy`, `matplotlib`等をインポート

4. **Cell 4-5: データの準備**
   - PPM形式のカラーチャート画像を読み込み
   - 参照画像（3×3と6×6のカラーチャート）を準備
   - 推論用データ（X）と正解データ（Y）を0-1に正規化して準備
   - データは`formatted_arrays.txt`として保存される

**ステップ2: ニューラルネットワークの学習（Cell 6）**

1. **モデル設定**
   ```python
   HIDDEN_DIM = 40  # 隠れ層の次元（40, 80, 120などに変更可能）
   ```

2. **モデル定義**
   - 4層の全結合ニューラルネットワーク
     - 入力層: 3次元（RGB）
     - 隠れ層1: HIDDEN_DIM次元（ReLU活性化）
     - 隠れ層2: HIDDEN_DIM次元（ReLU活性化）
     - 出力層: 3次元（RGB）

3. **学習パラメータ**
   - 損失関数: MSE + L1正則化
   - L1正則化係数: `lambda_l1 = 1e-6`
   - 最適化手法: Adam（学習率: 0.01）
   - エポック数: 50000
   - 500エポックごとに進捗を表示（MSE，L1，Loss）

4. **パラメータファイルの生成**
   - 学習完了後，`model_parameters.h`が自動生成される
   - 密行列形式で全層の重みとバイアスがC++配列として出力される
   - このファイルは`Arduino/AI_Model/`にコピーして使用

**ステップ3: CSR形式への変換（Cell 7）**

1. **前提条件**
   - Cell 6で学習したモデルが`model`変数として定義されている必要がある
   - 未定義の場合はエラーが発生

2. **プルーニング設定**
   ```python
   PRUNING_THRESHOLD = 0.01  # プルーニングのしきい値（固定）
   ```

3. **1層目のCSR変換**
   - 1層目（3 → HIDDEN_DIM）の重み行列を取得
   - プルーニング: 絶対値が0.01未満の重みを0に設定
   - CSR形式への変換:
     - `data_1`: 非ゼロ要素の値配列（float32）
     - `indices_1`: 列インデックス配列（uint16_t）
     - `indptr_1`: 行ポインタ配列（uint16_t，長さHIDDEN_DIM+1）
   - スパース性と圧縮率を表示

4. **2層目のCSR変換**
   - 2層目（HIDDEN_DIM → HIDDEN_DIM）の重み行列を取得
   - 同様にプルーニングとCSR形式への変換を実施
   - `data_2`, `indices_2`, `indptr_2`を生成

5. **パラメータファイルの生成**
   - `model_parameters_csr{HIDDEN_DIM}.h`として出力
   - 例: `model_parameters_csr40.h`, `model_parameters_csr80.h`
   - このファイルは`Arduino/AI_CSR/`にコピーして使用

**ステップ4: 量子化の適用（Cell 8，オプション）**
...
2. **量子化処理**
   - CSR形式の`data_1`と`data_2`に対してint8量子化を適用
   - スケールファクタの計算:
     - `scale_factor = 127.0 / max(abs(data))`
     - 各層ごとに独立して計算
   - 量子化: `quantized = round(data * scale_factor)`を-128〜127の範囲にクリッピング
   - 逆量子化誤差を計算して表示

3. **パラメータファイルの生成**
   - `model_parameters_csr_quantized{HIDDEN_DIM}.h`として出力
   - 例: `model_parameters_csr_quantized40.h`
   - このファイルは`Arduino/AI_CSR_Quantized/`にコピーして使用
...
3. **Cell 7を実行**してCSRパラメータを生成（`model_parameters_csr80.h`を生成）
4. **Cell 8を実行**して量子化パラメータを生成（オプション，`model_parameters_csr_quantized80.h`を生成）
5. **生成ファイルをArduinoフォルダにコピー**
...
生成されたパラメータファイルを対応するArduinoプロジェクトフォルダにコピーします：

| 生成ファイル | コピー先 | 用途 |
|------------|---------|------|
| `model_parameters.h` | `Arduino/AI_Model/` | 密行列形式モデル |
| `model_parameters_csr{HIDDEN_DIM}.h` | `Arduino/AI_CSR/` | CSR形式モデル |
| `model_parameters_csr_quantized{HIDDEN_DIM}.h` | `Arduino/AI_CSR_Quantized/` | CSR+量子化モデル |
...
各Arduinoプロジェクト内の`.ino`ファイルを開き，以下を確認・設定します：
...
**注意**: `HIDDEN_DIM`の値と読み込むパラメータファイルの次元が一致していない場合，正常に動作しません。

**ステップ3: コンパイルとアップロード**

1. Arduino IDEで対象の`.ino`ファイルを開く
2. ボード設定を「Arduino UNO R4 WiFi」に設定
3. 接続されているシリアルポートを選択
4. 「検証」ボタンでコンパイルエラーがないことを確認
5. 「アップロード」ボタンでArduinoに書き込む

**重要**: コンパイル時にはメモリ使用量が表示されます。SRAM使用量が32KBを超える場合はアップロードに失敗するため，その場合はより高い圧縮率の手法を適用する必要があります。
...
センサの感度は環境光や個体差により変動するため，測定前にキャリブレーションを行います。

1. 赤色タクトスイッチ（D2ピン）を押し，キャリブレーションモードに入る
2. 黒色サンプルをセンサにかざし，青色タクトスイッチ（D3ピン）を押して最小値を記録
3. 白色サンプルをセンサにかざし，青色タクトスイッチを押して最大値を記録
4. 赤色タクトスイッチを押し，キャリブレーションモードを終了

これらの値を基準として読み取り範囲を正規化することで，安定したRGB値の取得を可能とします。

**ステップ2: カラーチャートの読み取り**

キャリブレーション完了後，3×3のカラーチャートをセンサで読み取ります。

1. カラーチャートの各セルにセンサを順番にかざす
2. 青色タクトスイッチ（D3ピン）を押してRGB値を取得する
3. 取得したRGB値はニューラルネットワークによる推論を経て補正される
4. 全9セルの読み取りが完了するまで繰り返す

**ステップ3: PPM画像の出力**

取得したRGBデータは，シリアル通信を介してPC上のスキャナプログラムに送信され，PPM（Portable Pixel Map）形式の画像ファイルとして出力されます。PPM形式は非圧縮のラスタ画像フォーマットであり，画素値を直接比較できるため，後続のMSE評価に適しています。

出力されたPPMファイルは，実験条件ごとに以下のフォルダに整理して保存します：
- 密行列形式（40次元）: `ppm/nn40/`
- CSR形式（40次元）: `ppm/csr_40_0.01/`
- CSR形式（80次元）: `ppm/csr_80_0.01/`
- CSR+量子化（40次元）: `ppm/q+csr_40_0.01/`
- など

各実験条件につき複数回（10回程度）のスキャンを実施し，結果のばらつきを考慮します。
...
1. ブラウザで`mse_calculator.html`を開く
2. 参照画像（`ppm/reference_image1.ppm`）をドラッグ&ドロップする
3. スキャン結果のPPMファイルをドラッグ&ドロップする
4. 算出されたMSE値を記録する
5. 各実験条件のすべてのスキャン結果についてMSEを算出し，平均値・最小値・最大値を求める
...
2. **CSR圧縮**: プルーニング後にCSR形式へ変換したモデル（1層目と2層目の両方に適用）
3. **CSR＋量子化**: CSR圧縮に加えてint8量子化を適用したモデル（1層目と2層目の両方に適用）

これらのモデルについて，各隠れ層次元におけるMSEとメモリ使用量を比較し，段階的圧縮アプローチの有効性を検証しました。
