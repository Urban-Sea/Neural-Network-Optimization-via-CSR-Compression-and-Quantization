...
\subsection{パラメータ生成時間}

各条件におけるパラメータファイル（\texttt{.h}ファイル）の生成時間を表\ref{tab:results_generation_time}に示す．

\begin{table}[h]
    \centering
    \caption{パラメータファイル生成時間}
    \label{tab:results_generation_time}
    \begin{tabular}{|l|c|c|}
        \hline
        \textbf{圧縮手法} & \textbf{隠れ層次元} & \textbf{生成時間} \\
        \hline
        \hline
        \multicolumn{3}{|c|}{\textbf{非圧縮（密行列形式）}} \\
        \hline
        密行列形式 & 40 & -- \\
        \hline
        \hline
...
表\ref{tab:results_dense_detail}に，密行列形式（40次元）の詳細な測定結果を示す．

\begin{table}[h]
    \centering
    \caption{密行列形式（40次元）の詳細測定結果}
    \label{tab:results_dense_detail}
    \begin{tabular}{|c|c||c|c|}
        \hline
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
        \hline
        \multirow{4}{*}{40} & csr1.ppm & 600.00 & \multirow{4}{*}{追加測定予定} \\
        \cline{2-3}
        & csr2.ppm & 517.48 & \\
        \cline{2-3}
        & csr3.ppm & 1070.74 & \\
...
CSR＋量子化モデルについては，現時点で実験未実施である．
CSR圧縮のみでメモリ制約を満たせない場合，または隠れ層次元をさらに拡張する場合に実施予定である．
...
\begin{enumerate}
    \item 非圧縮（密行列形式，40次元）のMSE平均値は730.13であった．
    \item CSR圧縮（80次元）のMSE平均値は464.86であり，非圧縮（40次元）と比較して約36\%改善した．
    \item 隠れ層次元を40から80に拡張しても，CSR圧縮によりArduino Uno R4 WiFiのメモリ制約内で動作可能であった．
    \item CSR圧縮（40次元，120次元）およびCSR＋量子化モデルについては，追加測定を実施予定である．
\end{enumerate}
