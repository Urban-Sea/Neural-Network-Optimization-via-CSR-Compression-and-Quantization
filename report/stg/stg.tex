...
各条件におけるメモリ使用量を表\ref{tab:results_memory}に示す．
...
\begin{itemize}
    \item 非圧縮（密行列形式）：60次元まで動作可能，70次元でメモリ不足
    \item CSR圧縮：600次元まで動作可能（メモリ使用率67\%），800次元でメモリ不足
    \item CSR＋量子化：600次元まで動作可能（メモリ使用率66\%），800次元でメモリ不足
\end{itemize}

CSR圧縮により，非圧縮と比較して約10倍の次元拡張が可能となった．

\subsubsection{推論精度（MSE）}
40次元における各手法のMSE平均値を比較すると，非圧縮が595.95，CSR圧縮が518.58，CSR＋量子化が408.97であった．
CSR＋量子化は非圧縮と比較して約31\%のMSE改善を達成した．

測定済みの範囲において最良のMSE平均値は，CSR＋量子化（120次元）の244.83であった．
一方，CSR圧縮では次元を増加させてもMSEが単調に減少せず，400次元で864.40，600次元で670.69とばらつきが見られた．

\subsubsection{メモリ効率}
40次元における各手法のメモリ使用量を比較すると，非圧縮が14,524バイト（44\%），CSR圧縮が8,488バイト（25\%），CSR＋量子化が8,160バイト（24\%）であった．
CSR圧縮により約42\%，CSR＋量子化により約44\%のメモリ削減を達成した．

詳細な考察は次章で述べる．
なお，各実験条件における個別のスキャン結果は付録\ref{sec:appendix_detail}に示す．
...
70次元以上はメモリ不足によりコンパイルエラーが発生したため，測定を実施できなかった．
...
800次元はメモリ不足によりコンパイルエラーが発生したため，測定を実施できなかった．
...
160次元〜600次元は測定未実施，800次元はメモリ不足によりコンパイルエラーが発生した．
